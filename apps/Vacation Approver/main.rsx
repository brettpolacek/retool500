  ...
  >
    ...
    >
      ...
      <Column
        ...
        groupAggregationMode="none"
        hidden="true"
        key="approve_or_rejected_by"
        label="Approve or rejected by"
        placeholder="Enter value"
        position="center"
        size={100}
        summaryAggregationMode="none"
    ...
    <Button
      id="button2"
      disabled="{{table1.selectedRow.employee_remaining_days < table1.selectedRow.end_date - table1.selectedRow.start_date}}"
      marginType="normal"
    ...
    <MultiselectListbox
      ...
      value="{{ [self.values[0]] }}"
    >
      <Option id="52340" value="Option 1" />
      <Option id="ad3e5" value="Option 2" />
      <Option id="560e3" value="Option 3" />
    </MultiselectListbox>
    <Table
      id="table2"
    ...
    >
      ...
      <Column
        ...
        placeholder="Enter value"
        position="center"
        referenceId="financialChart"
        size={349}
        summaryAggregationMode="none"
        valueOverride="View Chart"
      >
        <Event
          event="clickCell"
          method="open"
      ...
      <Column
        ...
        size={100}
        summaryAggregationMode="none"
        valueOverride="{{chart1}}"
      />
      <Action id="93674" icon="bold/money-graph-bar" label="Financials">
        <Event
          event="clickAction"
          method="open"
          params={{ ordered: [] }}
    ...
    <PlotlyChart
        ...
        ordered: [
          {
            0: {
              ordered: [
                { label: "request_id" },
                { datasource: "{{getRequests.data['request_id']}}" },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#1E3A8A" },
                { colors: { ordered: [] } },
          ...
          },
          {
            1: {
              ordered: [
                { label: "id" },
                { datasource: "{{getRequests.data['id']}}" },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#3170F9" },
                { colors: { ordered: [] } },
          ...
          },
          {
            2: {
              ordered: [
                { label: "employee_id" },
                { datasource: "{{getRequests.data['employee_id']}}" },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#60A5FA" },
                { colors: { ordered: [] } },
          ...
          },
          {
            3: {
              ordered: [
                { label: "manager_id" },
                { datasource: "{{getRequests.data['manager_id']}}" },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#DBEAFE" },
                { colors: { ordered: [] } },
          ...
          },
          {
            4: {
              ordered: [
                { label: "approve_or_rejected_by" },
                {
                  datasource: "{{getRequests.data['approve_or_rejected_by']}}",
                },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#FDE68A" },
          ...
          },
          {
            5: {
              ordered: [
                { label: "employee_remaining_days" },
                {
                  datasource: "{{getRequests.data['employee_remaining_days']}}",
                },
                { chartType: "bar" },
                { aggregationType: "sum" },
                { color: "#F59E0B" },
      ...
      datasourceDataType="object"
      datasourceInputMode="javascript"
      datasourceJS="{{getRequests.data}}"
      skipDatasourceUpdate={true}
      xAxis="{{getRequests.data.start_date}}"
      xAxisDropdown="start_date"
    />
  </Frame>
</App>
